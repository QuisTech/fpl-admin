...
function evaluateCleanSheet(params: UtilityParameters, providers: VaastavProvider[]): { rmse: number, mae: number, spearman: number, loss: number } {
  let totalSqErr = 0;
  let totalAbsErr = 0;
  let count = 0;
  ...
  for (const provider of providers) {
    for (let gw = 1; gw <= 38; gw++) {
      ...
      for (const id of validPlayers) {
        ...
        let csMultiplier = pos === 'DEF' || pos === 'GKP' ? 4 : (pos === 'MID' ? 1 : 0);
        ...
        fixtures.forEach(fix => {
          ...
          expectedCSSum += expectedCsProb * csMultiplier;
        });

        const actualCS = provider.getActualCleanSheetPoints(id, gw);
        
        const err = expectedCSSum - actualCS;
        totalSqErr += err * err;
        totalAbsErr += Math.abs(err);
        count++;

        gwPredicted.push(expectedCSSum);
        gwActual.push(actualCS);
      }
...
async function runTraining() {
  ...
  let bestTrain = evaluateCleanSheet(bestParams, trainProviders);
  let bestVal = evaluateCleanSheet(bestParams, valProviders);
  ...
  for (let gen = 1; gen <= MAX_GENERATIONS; gen++) {
    ...
    for (let p = 0; p < POPULATION_SIZE; p++) {
      ...
      const metrics = evaluateCleanSheet(candidateParams, trainProviders);
