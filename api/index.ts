  ...
  try {
    ...
    for (const tid of teamIdList) {
      ...
      try {
        ...
        // Evaluate and save authentic user squad across all 3 fuels if tid is a valid team
        try {
          for (const uFuel of fuels) {
            const syncRes = await FPLService.syncTeam(tid, 'safe', 'free', uFuel, gwId, 'quant');
            if (syncRes && syncRes.squad && syncRes.squad.length >= 11) {
              const startingXI = syncRes.squad.filter((p: any) => (p.position_in_squad ?? 0) <= 11);
              const bench = syncRes.squad.filter((p: any) => (p.position_in_squad ?? 0) >= 12);
              const captain = syncRes.squad.find((p: any) => p.isCaptain || p.is_captain) || (startingXI.length > 0 ? startingXI[0] : null);
              const viceCaptain = syncRes.squad.find((p: any) => p.isViceCaptain || p.is_vice_captain);
              ...
              const uKey = `user_synced_squad_${uFuel}`;
              newGwHistory[uKey] = {
                key: uKey,
                fuel: uFuel,
                fuelLabel: uFuel === 'eye-test' ? 'Eye Test' : uFuel === 'native' ? 'Native FPL' : 'FPLForm',
                scenario: 'user',
                scenarioLabel: syncRes.managerInfo?.teamName || 'Synced FPL Squad',
                riskLabel: 'HUMAN',
                isUserSquad: true,
                autoGenerated: true,
                timestamp: now,
          ...
          delete newGwHistory['user_synced_squad'];
        } catch (err: any) {
          console.warn(`[AutoSnapshot] Notice evaluating user squad for team ${tid}:`, err.message);
        }
